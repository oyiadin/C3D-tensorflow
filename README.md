# C3D-tensorflow demo usage


usage: python3 demo.py \[action \[in_file\] \[out_file\]\]

available actions:

* gen_list \[in_file\] \[out_file\]

to sample from the `in_file` randomly and save the selected samples into the `out_file`

* demo_video \[in_file\] \[out_file\]

to generate a short video and labelling them with our pretrained model.

you should gather 20 samples together and save it to `in_file` (can be automatically generated by action `gen_list`)

* *(run without any arguments)*

the program would capture images from your camera, and predict it through the model, then make a label in the opencv2 window.
